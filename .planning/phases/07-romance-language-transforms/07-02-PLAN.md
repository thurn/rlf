...
must_haves:
  truths:
    ...
    - "Contractions always produce lowercase output regardless of input"
  artifacts:
    ...
    - path: "crates/rlf/tests/interpreter_transforms.rs"
      provides: "Tests for French and Italian transforms including elision and lowercase validation"
...
**CRITICAL: Contractions must always output lowercase** per locked decision "Capitalization handled via separate @cap transform (contractions always lowercase)". All contraction lookup tables return lowercase strings only.
...
fn french_de_elision_preserves_lowercase() {
    ...
    assert!(result.starts_with("de l'"), "Elided contraction must be lowercase");
}
...
```
  ...
  <done>FrenchLe, FrenchUn, FrenchDe, FrenchAu, FrenchLiaison variants exist, elision handling works, contractions always lowercase</done>
</task>
...
fn italian_di_elision_preserves_lowercase() {
    ...
    assert_eq!(result, "dell'Amico");
    assert!(result.starts_with("dell'"), "Elided contraction must be lowercase");
}
...
```
  ...
  <done>ItalianIl, ItalianUn, ItalianDi, ItalianA variants exist, sound-based lookup works, contractions always lowercase</done>
</task>
...
- **Contractions always produce lowercase output** (validated by dedicated tests)
</success_criteria>
