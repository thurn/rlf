...
must_haves:
  truths:
    ...
    - "TransformRegistry stub exists with get/has_transform methods for Phase 3"
  artifacts:
...
2. Create `crates/rlf/src/interpreter/mod.rs`:
   - Declare submodules: `mod error; mod registry; mod context; mod plural; mod transforms;`
   - Re-export: `pub use error::EvalError; pub use registry::PhraseRegistry; pub use context::EvalContext; pub use transforms::TransformRegistry;`
...
5. Create `crates/rlf/src/interpreter/transforms.rs` with TransformRegistry stub:
   ```rust
   use crate::types::Value;
   use crate::interpreter::EvalError;

   /// Transform function signature.
   /// Takes value to transform, optional context, and language code.
   ...
   pub struct TransformRegistry {
       ...
       universal: std::collections::HashMap<String, TransformFn>,
       /// Language-specific transforms (keyed by lang -> transform_name -> fn)
       language_specific: std::collections::HashMap<String, std::collections::HashMap<String, TransformFn>>,
   }
...
6. Update `crates/rlf/src/lib.rs`:
   - Add `pub mod interpreter;`
   - Add to exports: `pub use interpreter::{EvalError, PhraseRegistry, EvalContext, TransformRegistry};`
  </action>
  <verify>
