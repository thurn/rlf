...
This enables `@der:acc` where `:acc` is resolved as the literal string "acc", or `@der:case` where `case` could be a parameter containing a case string.
  ...
  <verify>Run `cargo check -p rlf`. Existing tests still pass: `cargo test -p rlf`</verify>
  <done>Context selector is resolved to Value and passed to transform execute method</done>
</task>
